...
def build_overview_latex(
    ...
    header = (
        "        Run & Time (s) & Vars & Int (free/total) & Constr. & Gap (\\%) & Status \\\\"
    )
    body_lines = []
...
def run_all() -> None:
    ...
    for run_dir in run_dirs:
        try:
            ...
            if study is not None:
                per_study.setdefault(study, []).append((_study_run_label(run_dir, study), stats))
    ...
    # Per-study overview: only that study's runs, written into the study folder.
    ...
    print(
        f"\nWrote {len(rows)} per-run tables + global overview + "
        f"{len(per_study)} per-study overviews."
    )
    print(f"  global: {overview_path}")
    if n_missing:
        print(
