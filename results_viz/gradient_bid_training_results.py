...
scenario in each regime, takes that scenario's saved final block bid profiles
from ``bid_history[-1]``, solves economic dispatch, and saves one four-panel
figure per (regime, bidding block).
"""
...
class GradientBidFirstScenarioRegimeVisualizer:
    ...
    def __init__(
        ...
        self.dispatches, self.block_dispatches, self.clearing_prices = self._solve_regime_dispatch()

    def _initialize_block_metadata(self) -> None:
        """
        Use result metadata when present, otherwise fall back to the ED model's
        scenario-derived ordering. New direct-bid results store bids by block,
        while physical dispatch remains generator-level.
        """
        ...
        self.block_names = list(self.results.get("block_names", [])) or mapping_model.get_block_names()
        self.physical_generator_names = (
            list(self.results.get("physical_generator_names", []))
            or mapping_model.get_physical_generator_names()
        )
        self.generator_names = list(self.results.get("generator_names", [])) or list(self.physical_generator_names)
        self.block_to_physical = (
            dict(self.results.get("block_to_physical", {}))
            or mapping_model.get_block_to_physical_mapping()
        )

        raw_blocks_by_generator = self.results.get("blocks_by_generator")
        if raw_blocks_by_generator:
            self.blocks_by_generator = {
                int(generator_idx): [int(block_idx) for block_idx in block_indices]
                for generator_idx, block_indices in raw_blocks_by_generator.items()
            }
        else:
            self.blocks_by_generator = mapping_model.get_blocks_by_generator()

        self.physical_to_block_indices = [
            list(self.blocks_by_generator[i])
            for i in range(len(self.physical_generator_names))
        ]

        self.block_to_physical_idx = [
            self.physical_generator_names.index(self.block_to_physical[block_name])
            for block_name in self.block_names
        ]

        if not self.block_names:
            raise ValueError("Could not infer bidding block names")
        if any(len(blocks) == 0 for blocks in self.physical_to_block_indices):
            raise ValueError("Every physical generator must have at least one block for visualization")
    ...
    def _build_regime_scenarios_df(self) -> pd.DataFrame:
        ...
        for regime_scenario_idx, regime in enumerate(regimes):
            ...
            for block_idx, block_name in enumerate(self.block_names):
                ...
                for profile_col in (f"{block_name}_profile", f"{block_name}_cap_profile"):
                ...
                if f"{block_name}_profile" not in row and f"{block_name}_cap_profile" in row:
                    row[f"{block_name}_profile"] = list(row[f"{block_name}_cap_profile"])

                bid_profile = self.final_bids[source_idx, block_idx, :]
                bid_profile_list = bid_profile.astype(float).tolist()
                row[f"{block_name}_bid_profile"] = bid_profile_list
                row[f"{block_name}_bid"] = float(bid_profile_list[0])
    ...
    def _compute_p_init(self) -> List[List[float]]:
        ...
        for block_name in self.block_names:
            ...
            cost_bid_df[f"{block_name}_bid"] = cost
            cost_bid_df[f"{block_name}_bid_profile"] = [
    ...
    def _solve_regime_dispatch(
        self,
    ) -> tuple[List[List[List[float]]], List[List[List[float]]], List[List[float]]]:
        ...
        block_dispatches_all: List[List[List[float]]] = []
        ...
        for scenario_idx in range(len(self.regime_scenarios_df)):
            ...
            block_dispatches_all.append(block_dispatches[0])
            prices_all.append(prices[0])

        return dispatches_all, block_dispatches_all, prices_all

    def _available_capacity_profile(self, scenario_idx: int, block_name: str) -> np.ndarray:
        row = self.regime_scenarios_df.iloc[scenario_idx]
        for profile_col in (f"{block_name}_cap_profile", f"{block_name}_profile"):
        ...
        return np.full(self.num_time_steps, float(row[f"{block_name}_cap"]), dtype=float)

    def plot_regime_block(self, scenario_idx: int, block_idx: int, show: bool = False) -> None:
        row = self.regime_scenarios_df.iloc[scenario_idx]
        ...
        physical_idx = self.physical_generator_names.index(physical_name)
        time_axis = np.arange(self.num_time_steps)

        demand = np.asarray(row["demand_profile"], dtype=float)
        capacity = self._available_capacity_profile(scenario_idx, block_name)
        bid = np.asarray(row[f"{block_name}_bid_profile"], dtype=float)
        price = np.asarray(self.clearing_prices[scenario_idx], dtype=float)
        block_dispatch = np.asarray(self.block_dispatches[scenario_idx], dtype=float)[:, block_idx]
        physical_dispatch = np.asarray(self.dispatches[scenario_idx], dtype=float)[:, physical_idx]
        marginal_offer = bid + 2.0 * self.beta_smooth * block_dispatch

        fig, axes = plt.subplots(4, 1, figsize=(11, 10), sharex=True)
        fig.suptitle(
            f"First Scenario in Regime - {regime} - {block_name} "
            f"({physical_name}, scenario {source_idx})",
            fontsize=14,
        )
        ...
        axes[1].set_title(f"{block_name} Available Capacity")
        ...
        if abs(self.beta_smooth) > 0.0:
            axes[2].plot(
                ...
                linewidth=1.6,
                linestyle=":",
                label=f"{block_name} bid + 2 beta block dispatch",
            )
        ...
        axes[3].plot(
            ...
            marker="o",
            linewidth=2.0,
            label=f"{block_name} block dispatch",
        )
        axes[3].plot(
            time_axis,
            physical_dispatch,
            ...
            linewidth=1.5,
            linestyle="--",
            label=f"{physical_name} physical dispatch",
        )
        axes[3].set_ylabel("MW")
        axes[3].set_xlabel("Time step")
        axes[3].set_title("Block Dispatch and Owning Physical Generator Dispatch")
        ...
        out = regime_dir / f"gradient_bid_first_scenario_{block_name}.png"
    ...
    def run(self, show: bool = False) -> None:
        ...
        print(f"Physical generators: {', '.join(self.physical_generator_names)}")
        print(f"Bidding blocks: {', '.join(self.block_names)}")
        ...
        for scenario_idx in range(len(self.regime_scenarios_df)):
            for block_idx in range(len(self.block_names)):
                self.plot_regime_block(scenario_idx=scenario_idx, block_idx=block_idx, show=show)
