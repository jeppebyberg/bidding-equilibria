...
from xXgraveyard.models.gradient_based.sensitivities.kkt_sensitivity import (
    EDSolution,
    EDParameters,
    compute_market_sensitivities,
)
from xXgraveyard.models.gradient_based.sensitivities.profit_sensitivity import (
    ProfitParameters,
    compute_profit_sensitivities,
)
from xXgraveyard.models.gradient_based.sensitivities.policy_sensitivity import (
    PolicyParameters,
...
class GradientPolicyTrainingKKTNNMS:
    ...
    def __init__(
        ...
        if hidden_init_scale < 0:
            raise ValueError(f"hidden_init_scale must be nonnegative, got {hidden_init_scale}")
        ...
        self.hidden_init_scale = float(hidden_init_scale)
    ...
    def _clip_alpha_bids_and_jacobian(
        self,
        alpha_bids: np.ndarray,
        dalpha_dtheta: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray]:
        alpha_arr = np.asarray(alpha_bids, dtype=np.float64)
        jac = np.asarray(dalpha_dtheta, dtype=np.float64)
        clipped = self._clip_alpha_array(alpha_arr)
        active = np.ones(alpha_arr.shape, dtype=np.float64)
        if self.alpha_min is not None:
            active[alpha_arr <= float(self.alpha_min)] = 0.0
        if self.alpha_max is not None:
            active[alpha_arr >= float(self.alpha_max)] = 0.0
        active_time_major = active.T.reshape(-1)
        if jac.shape[0] != active_time_major.size:
            raise ValueError(
                "dalpha_dtheta row count must match flattened alpha bids: "
                f"{jac.shape[0]} != {active_time_major.size}"
            )
        return clipped, jac * active_time_major[:, None]
    ...
    def compute_player_gradient(
        ...
        for s in range(self.num_scenarios):
            ...
            alpha_bids, dalpha_dtheta = self._clip_alpha_bids_and_jacobian(
                alpha_bids,
                dalpha_dtheta,
            )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            for player in self.players_config:
                ...
                # dispatch_summary = self._player_dispatch_price_summary(player_id)
                # print(f"    post-update dispatched MWh: {dispatch_summary['dispatched_mwh']:.6f}")
                # if dispatch_summary["has_meaningful_dispatch"]:
                #     print(f"    post-update dispatch-wtd price: {dispatch_summary['dispatch_weighted_price']:.6f}")
                #     print(f"    post-update dispatch-wtd cost : {dispatch_summary['dispatch_weighted_cost']:.6f}")
                # else:
                #     print("    post-update dispatch-wtd price/cost: n/a")
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hidden_init_scale": self.hidden_init_scale,
    ...
    def _alpha_summary_for_generators(self, generator_indices: List[int]) -> Dict[str, float]:
        ...
        for s in range(self.num_scenarios):
            for gen_idx in generator_indices:
                ...
                profile = [float(v) for v in self.scenarios_df.at[s, f"{gen_name}_bid_profile"]]
    ...
    def _player_dispatch_price_summary(self, player_id: int) -> Dict[str, float]:
        ...
        for s in range(self.num_scenarios):
            for t in range(self.num_time_steps):
                price = float(clearing_prices[s][t])
                for gen_idx in self._controlled_generators(player_id):
                    p = float(dispatches[s][t][int(gen_idx)])
                    if abs(p) <= dispatch_tolerance:
                        continue
                    cost = float(self.cost_vector[int(gen_idx)])
                    total_dispatch += p
                    weighted_price += price * p
                    weighted_cost += cost * p
        if abs(total_dispatch) <= dispatch_tolerance:
            return {
                "dispatched_mwh": 0.0,
                "dispatch_weighted_price": float("nan"),
                "dispatch_weighted_cost": float("nan"),
                "has_meaningful_dispatch": False,
            }
        return {
            "dispatched_mwh": float(total_dispatch),
            "dispatch_weighted_price": float(weighted_price / total_dispatch),
            "dispatch_weighted_cost": float(weighted_cost / total_dispatch),
            "has_meaningful_dispatch": True,
        }
...
if __name__ == "__main__":
    ...
    fb.save_feature_normalizer_stats("results/feature_normalizer_stats_gradient.json")
