...
class ReLUBoundsComputer(PoATighteningMain):
    ...
    @staticmethod
    def _relu_fixed_binaries(report: dict[str, Any]) -> dict[str, dict[str, Any]]:
        ...
        for generator_name, entries in (report.get("nn_relu_bounds", {}) or {}).items():
            for key, details in (entries or {}).items():
                ...
                if status == "active":
                    ...
                    continue
                fixed.setdefault(str(generator_name), {})[str(key)] = {
                    "fixed_value": fixed_value,
                    "status": status,
                    "time_idx": int(details.get("time_idx")),
                    "linear_idx": int(details.get("linear_idx")),
                    "node": int(details.get("node")),
                }
        return fixed
    ...
    def run_relu_bounds(
        ...
        output_path = output_path or DEFAULT_TIGHTENING_OUTPUT_PATHS["relu_bounds"]
        ...
        report.setdefault("metadata", {})["runtime_seconds"] = time.perf_counter() - start
